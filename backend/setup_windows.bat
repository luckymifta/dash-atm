...
if exist ".env" (
    ...
    if exist ".env.template" (
        ...
        echo.
        echo WARNING: Please edit the .env file with your actual database credentials!
        echo.
        ...
        echo ERROR: .env.template not found. Creating a basic .env file...
...
if %errorlevel% neq 0 (
    echo ERROR: Python not found. Please install Python 3.7+ and add it to your PATH.
    pause
    exit /b 1
) else (
    python --version
...
if %errorlevel% neq 0 (
    echo.
    echo ERROR: Setup incomplete. Please fix the issues above before running the ATM script.
    pause
    exit /b 1
)
